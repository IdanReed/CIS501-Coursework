...
namespace Websocket_Server
{
    ...
    class Chat : WebSocketBehavior
    {
        ...
        {
            foreach(string histMsg in history)
            {
                string joinedID = null;
                foreach (object something in Sessions.IDs)
                {
                    joinedID = Convert.ToString(something);
                }
                if(joinedID != null)
                {

                    Sessions.SendTo(joinedID, histMsg);
                }
            }
        }
