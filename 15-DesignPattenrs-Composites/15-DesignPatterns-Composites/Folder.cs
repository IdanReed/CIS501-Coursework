...
namespace DesignPatterns_Composites
{
    ...
    public class Folder : Entry
    {
        ...
        public Entry search(string path)
        {
            ...
            if (!directory.ContainsKey(names[0]))
            {
                return null;
            }
            Entry matchingEntry = directory[names[0]];
            if(matchingEntry != null)
            {
                
                if (matchingEntry is Shortcut)
                {
                    Entry nextEntry = matchingEntry;
                    while (nextEntry is Shortcut)
                    {
                        nextEntry = ((Shortcut)matchingEntry).Link;
                    }
                    if(nextEntry is TextFile)
                    {
                        return (TextFile)nextEntry;
                    }
                    else if (nextEntry is Folder)
                    {
                        StringBuilder pathNew = new StringBuilder();
                        for (int i = 1; i < names.Count(); i++)
                        {
                            pathNew.Append(names[i]);
                        }
                        return ((Folder)nextEntry).search(pathNew.ToString());
                    }

                }
                else if (matchingEntry is TextFile)
                {
                    return (TextFile)matchingEntry;
                }else if (matchingEntry is Folder)
                {
                    StringBuilder pathNew = new StringBuilder();
                    for(int i = 1; i < names.Count(); i++)
                    {
                    ...
                    return ((Folder)matchingEntry).search(pathNew.ToString());
                }
                return null;
            }
